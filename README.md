...
* `website/app.py`: This is an application created with Flask to launch the website. It is used to connect the search engine contained in `app_helper.py` as a backend and `search_engine.html` as the frontend. `search_engine.html` is based on an html template obtained from [Materialize](https://materializecss.com).

## How to Use and Deploy

The search engine can be deployed by opening a terminal in the website folder and running the following command: 
...
![](/home/nelsonalejandrov/Pictures/welcome.png)
...
![](/home/nelsonalejandrov/Pictures/search.png)

**Prerequisites:**

* The app requires having Python installed and the following libraries: pandas, sklearn, joblib and Flask. One option is installing a Python manager such as Anaconda since it includes the required packages preinstalled for you. 
